...
static int export_oxfd_features( char*, struct feature*, int, float, float, float, float);
...
static int export_lowe_features( char*, struct feature*, int, float, float, float, float);
...
/*
   ...
   */
int export_features( char* filename, struct feature* feat, int n, float x1, float y1, float x2, float y2)
{
    int r, type;
    ...
    {
        case FEATURE_OXFD:
            r = export_oxfd_features( filename, feat, n, x1,y1,x2,y2 );
            break;
        case FEATURE_LOWE:
            r = export_lowe_features( filename, feat, n, x1,y1,x2,y2 );
            break;
        default:
...
int export_all_features( char* filename, struct feature* feat, int n)
{
...
@return Returns 0 on success or 1 on error
*/
static int export_oxfd_features( char* filename, struct feature* feat, int n, float x1, float y1, float x2, float y2 )
{
    FILE* file;
    int i, j, d;
    ...
    for( i = 0; i < n; i++ )
    {
        if(feat[i].x>=x1 && feat[i].x<=x2 && feat[i].y>=y1 && feat[i].y<=y2){  
...
static int import_lowe_features( char* filename, struct feature** features )
{
    ...
    if(!fscanf(file, "%s", img_name)){
        fprintf( stderr, "img_name, Warning: file read error, %s, line %d\n",
                __FILE__, __LINE__ );
        return -1;
    }
...
@return Returns 0 on success or 1 on error
*/
static int export_lowe_features( char* filename, struct feature* feat, int n, float x1, float y1, float x2, float y2 )
{
    FILE* file;
    int i, j, d;
